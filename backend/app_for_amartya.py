...
# -------------------------------------------------------------------------
# FEATURE MATRIX (shared by prediction + analytics)
# -------------------------------------------------------------------------
# Order MUST match the model's training feature order.
FEATURE_NAMES = [
    "Hour", "Day", "Month", "Year", "Weekday",
    "Temperature", "Dew Point", "Humidity", "Wind Speed", "Wind Dir",
    "Cloud Cover", "Precipitation", "Is Holiday", "Is Weekend"
]


def build_feature_matrix(date_dt, weather_data):
    """Build the 24x14 raw feature matrix for a single day."""
    start_date = date_dt.strftime("%Y-%m-%d")
    base_time = date_dt.replace(hour=0, minute=0, second=0, microsecond=0)
...
# -------------------------------------------------------------------------
def predict_single_day(date_dt, weather_data=None):
    start_date = date_dt.strftime("%Y-%m-%d")

    # Allow callers to pass weather they already fetched (avoids a duplicate
    # Open-Meteo call when the same day is also used for analytics).
    ...
    features = build_feature_matrix(date_dt, weather_data)
...
def predict_weekly():
    try:
        ...
        # Keep full HOURLY resolution across the whole range so the chart can
        # be scrolled / zoomed down to each individual hour of each day.
        while current_day <= end_dt:
            daily_predictions = predict_single_day(current_day)
            response_data.extend(daily_predictions)
            current_day += timedelta(days=1)
...
def predict_monthly():
    try:
        ...
        # Return one point per DAY across the full range (supports up to ~3
        # months) instead of a single collapsed total, so the chart shows each
        # day and can be scrolled / zoomed.
        while current_day <= end_dt:
            daily_predictions = predict_single_day(current_day)
            daily_total = sum(
                float(item["value"])
...
# -------------------------------------------------------------------------
# ANALYTICS APIs (real, model + weather derived)
# -------------------------------------------------------------------------
_feature_importance_cache = None


@app.route("/analytics/environmental", methods=["GET"])
def analytics_environmental():
    """Hourly weather (temp/humidity) joined with predicted load for one day."""
    if model is None:
        return jsonify({"error": "model not loaded"}), 500
    try:
        date_epoch = request.args.get("date")
        date_dt = (
            datetime.fromtimestamp(int(date_epoch))
            if date_epoch else datetime.now()
        )

        start_date = date_dt.strftime("%Y-%m-%d")
        weather_data = fetch_weather(start_date, start_date)
        preds = predict_single_day(date_dt, weather_data=weather_data)
        ...
        for i, w in enumerate(weather_data):
            ...
            rows.append({
                "hour": f"{i:02d}:00",
                "temperature": round(float(w["temp"]), 1),
                "humidity": round(float(w["humidity"]), 1),
                "load": round(float(preds[i]["value"]), 2)
            })

        # Add min-max normalized (0-100) copies of each series. The three
        # series live on wildly different scales (load ~thousands MW vs
        # temp/humidity ~tens), so on a single axis temp/humidity look flat.
        # Normalizing lets the chart show each one's variation; the raw values
        # above are kept for the tooltip. This only shapes the API response —
        # the model's prediction math is unchanged.
        for field in ("temperature", "humidity", "load"):
            vals = [r[field] for r in rows]
            lo, hi = (min(vals), max(vals)) if vals else (0, 0)
            rng = hi - lo
            for r in rows:
                r[f"{field}_pct"] = (
                    round((r[field] - lo) / rng * 100, 1) if rng > 0 else 50.0
                )

        return jsonify({"status": "success", "date": start_date, "data": rows})
    except Exception as e:
        return jsonify({"error": str(e)}), 500


@app.route("/analytics/correlation", methods=["GET"])
def analytics_correlation():
    """Scatter of (temperature, humidity, predicted load) per hour over a range."""
    if model is None:
        return jsonify({"error": "model not loaded"}), 500
    try:
        start_epoch = request.args.get("start_date")
        ...
        start_dt = (
            datetime.fromtimestamp(int(start_epoch))
            if start_epoch else datetime.now()
        )
        end_dt = (
            datetime.fromtimestamp(int(end_epoch))
            if end_epoch else start_dt + timedelta(days=6)
        )

        # Bound the payload / number of weather calls.
        ...
        while current <= end_dt:
            ...
            try:
                weather_data = fetch_weather(sd, sd)
                preds = predict_single_day(current, weather_data=weather_data)
            except Exception:
                # A day outside the weather horizon shouldn't kill the whole
                # chart — just skip it and keep the days that do resolve.
...
def analytics_feature_importance():
    """Occlusion (mean-ablation) importance for each input feature.

    The model is a neural network (no built-in feature_importances_), so for
    every feature we replace its column with the dataset mean, re-run the
    model, and measure the average absolute change in predicted load. Larger
    change => more important. Averaged over several sample days and cached.
    """
    ...
    try:
        if _feature_importance_cache is not None and not request.args.get("refresh"):
            return jsonify(_feature_importance_cache)

        # Sample several days within the forecast horizon for a stable estimate.
        base = datetime.now()
        mats = []
        for offset in range(5):
            ...
            try:
                weather_data = fetch_weather(sd, sd)
                features = np.array(
                    build_feature_matrix(day, weather_data), dtype=float
                )
        ...
        if not mats:
            return jsonify({
                "error": "could not compute importance (weather unavailable)"
            }), 502

        feature_means = np.concatenate(mats, axis=0).mean(axis=0)
        n_features = len(FEATURE_NAMES)

        base_preds = [
            np.array(postprocess(model.predict(preprocess(m.tolist()))))
            for m in mats
        ]
        ...
        for j in range(n_features):
            ...
            for idx, m in enumerate(mats):
                occluded = m.copy()
                occluded[:, j] = feature_means[j]
                pred = np.array(
                    postprocess(model.predict(preprocess(occluded.tolist())))
                )
                diffs.append(float(np.mean(np.abs(pred - base_preds[idx]))))
        ...
        if total <= 0:
            ...
            pct = [round(float(v) / total * 100, 2) for v in importances]

        feats = sorted(
            [
                {"feature": FEATURE_NAMES[j], "importance": pct[j]}
                for j in range(n_features)
            ],
        ...
        result = {
            "status": "success",
            "method": "occlusion (mean-ablation)",
            "features": feats
        }
