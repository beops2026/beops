...
import { formatEpochLabel, formatEpochFull, getValueUnit } from "@/lib/forecast";
...
const THEMES = {
  ...
  weekly:    { color: "#22d3ee", gradientId: "grad-weekly",    label: "MWh" },
...
const CHART_TYPE_META = {
  area:     { icon: AreaIcon,   label: "Area",     tooltip: "Area chart — volume & trend" },
  line:     { icon: LineIcon,   label: "Line",     tooltip: "Line chart — precise trend" },
  bar:      { icon: BarChart3,  label: "Bar",      tooltip: "Bar chart — comparative value" },
  composed: { icon: Layers,     label: "Composed", tooltip: "Composed chart — bar & line hybrid" },
  stepped:  { icon: Activity,   label: "Stepped",  tooltip: "Stepped line chart — discrete stages" },
  scatter:  { icon: Crosshair,  label: "Scatter",  tooltip: "Scatter plot — data density" },
  pie:      { icon: PieIcon,    label: "Pie",      tooltip: "Pie chart — total shares" },
  radar:    { icon: RadarIcon,  label: "Radar",    tooltip: "Radar chart — cycle profile" },
};
...
const CHART_EXPLANATIONS = {
  hourly: {
    ...
    composed: "Composed view overlays intraday load volume with a smooth trendline.",
    stepped:  "Stepped view displays discrete load changes at hourly intervals.",
    scatter:  "Scatter view shows the distribution density of MW load observations.",
  },
  weekly: {
    ...
    radar:    "Radar view illustrates the load distribution pattern over the 7-day cycle.",
  },
  monthly: {
...
function ForecastTooltip({ active, payload, forecastType }) {
  ...
  return (
    <div className="rounded-xl bg-[#0d0d14]/95 border border-white/10 px-4 py-3 shadow-2xl backdrop-blur-md text-xs">
      <p className="text-zinc-400 mb-1.5">{formatEpochFull(time, forecastType)}</p>
...
function renderChart({ chartType, data, forecastType }) {
  ...
  const interval = forecastType === "hourly" ? 2
    : forecastType === "monthly" ? Math.max(1, Math.floor(data.length / 12))
    : "preserveStartEnd";
  ...
  const isTimeSeries = ["area", "line", "bar", "composed", "stepped"].includes(chartType);
  ...
  const referenceAreaAndLine = isTimeSeries ? (
    <>
      ...
      <ReferenceLine
        ...
        strokeDasharray="3 3"
        opacity={0.3}
        label={{ value: "Peak Risk Zone", fill: "#f43f5e", fontSize: 9, position: "insideBottomRight" }}
      />
    </>
  ) : null;
  ...
  const composedData = data.map((d, i) => {
    ...
    const avg = subset.reduce((sum, item) => sum + item.value, 0) / subset.length;
    return {
      ...d,
      movingAvg: avg,
    };
  ...
  if (chartType === "area") {
    return (
      <AreaChart {...common}>
        ...
        <XAxis dataKey={xKey} tickFormatter={tickFmt} {...axisProps} interval={interval} />
        <YAxis {...axisProps} tickFormatter={yFormatter} width={55} domain={['dataMin - 100', 'dataMax + 100']} />
  ...
  if (chartType === "line") {
    return (
      <LineChart {...common}>
        ...
        <CartesianGrid strokeDasharray="3 3" stroke="#ffffff06" />
        <XAxis dataKey={xKey} tickFormatter={tickFmt} {...axisProps} interval={interval} />
        <YAxis {...axisProps} tickFormatter={yFormatter} width={55} domain={['dataMin - 5%', 'dataMax + 5%']} />
        <Tooltip content={<ForecastTooltip forecastType={forecastType} />} />
        <ReferenceLine
          y={avgValue}
          stroke={`${color}40`}
  ...
  if (chartType === "composed") {
    return (
      <ComposedChart data={composedData} margin={common.margin}>
        ...
        />
        {referenceAreaAndLine}
        <Bar dataKey="value" fill={`${color}25`} radius={[3, 3, 0, 0]} barSize={20} />
        <Line
          type="monotone"
          dataKey="movingAvg"
          stroke={color}
    ...
    return (
      <ScatterChart {...common}>
        ...
        <XAxis
          ...
          interval={interval}
        />
        <YAxis type="number" dataKey="value" {...axisProps} tickFormatter={yFormatter} width={55} domain={['dataMin - 5%', 'dataMax + 5%']} />
        <Tooltip content={<ForecastTooltip forecastType={forecastType} />} />
    ...
    return (
      <PieChart margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
        <Tooltip
          ...
          contentStyle={{ backgroundColor: "#0d0d14", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12 }}
          itemStyle={{ color: "#fff" }}
        />
        <Pie
          data={pieData}
  ...
  if (chartType === "radar") {
    ...
    return (
      <RadarChart cx="50%" cy="50%" outerRadius="75%" data={radarData}>
        ...
        <PolarRadiusAxis angle={30} domain={[0, maxValue]} tick={{ fill: "#6b7280", fontSize: 9 }} />
        <Radar
          name="Load"
          dataKey="value"
          stroke={color}
  ...
  return (
    <div className="flex flex-col items-center justify-center h-64 gap-4 text-zinc-500">
      ...
      <div className="text-center">
        <p className="text-sm font-medium text-zinc-400">No forecast data yet</p>
        <p className="text-xs text-zinc-600 mt-1">Select a date range and click Generate Forecast</p>
      </div>
    </div>
  );
}
...
export default function ForecastChart({ data, forecastType, loading }) {
  ...
  const currentAllowed = ALLOWED_CHART_TYPES[forecastType] || ["line", "area", "bar"];
  const activeType = currentAllowed.includes(chartType) ? chartType : currentAllowed[0];

  if (loading) return <div className="p-2"><ChartSkeleton /></div>;
